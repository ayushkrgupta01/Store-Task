...
const AllStores = () => {
  ...
  const filtered = stores.filter((s) => {
    ...
    return (
      ...
      String(s.StateName || s.State || "").toLowerCase().includes(q) ||
      String(s.CityName || s.City || "").toLowerCase().includes(q) ||
      String(s.PAN || s.PanNumber || s.PanNo || "").toLowerCase().includes(q) ||
      String(s.Aadhar || s.AadharNumber || s.AadharNo || "").toLowerCase().includes(q)
    );
  });
  ...
  return (
    <div className="p-4 min-h-screen bg-gray-50">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">All Stores</h1>
          <p className="text-gray-500 mt-1">Browse, search and manage all stores</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-x-auto ring-1 ring-gray-100">
              ...
              {filtered.length > 0 ? (
                filtered.map((store, index) => (
                  <tr key={store.StoreId || index} className="hover:bg-gray-50 text-sm sm:text-[15px]">
                    <td className="p-3 border-t truncate">{getValue(store, ["StoreName"])}</td>
                    <td className="p-3 border-t truncate">{getValue(store, ["Email"])}</td>
                    <td className="p-3 border-t truncate">{getValue(store, ["Phone"])}</td>
                    <td className="p-3 border-t truncate">{getValue(store, ["StateName", "State", "state"])}</td>
                    <td className="p-3 border-t truncate">{getValue(store, ["CityName", "City", "city"])}</td>
                    <td className="p-3 border-t font-mono truncate">{getValue(store, ["PAN", "PANNumber", "PanNo"])}</td>
                    <td className="p-3 border-t font-mono truncate">{getValue(store, ["Aadhar", "AadharNumber", "AadharNo"])}</td>
