...
function UpdateCustomer() {
  ...
  // Memoized function for fetching data on component load
  const fetchCustomerData = useCallback(async () => {
    ...
    try {
      const [customerRes, productRes] = await Promise.all([
        axios.get(`${BACKEND_BASE_URL}/GetById/${id}`),
        axios.get(`${BACKEND_BASE_URL}/GetProductService`),
      ]);
      ...
      if (customer) {
        ...
        router.back(); // Redirect if customer doesn't exist
      }

      setProductServices(productRes.data || []);
    } catch (err) {
  ...
  const handleFileUpload = async (e, uploadType) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.error || "Upload failed. Please check the network."
      );
    } finally {
  ...
  const getInputIcon = (name) => {
    const iconMap = {
      customer_name: FaUser,
      customer_email: FaEnvelope,
      customer_phone: FaPhone,
      Customer_AadharNumber: FaIdCard,
      Customer_PanNumber: FaIdCard,
      Customer_ProductAmount: FaMoneyBill,
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    {formData.customer_aadhar && (
                      <div className="relative mt-4 w-full h-auto max-h-48 overflow-hidden rounded-md shadow-sm border border-gray-200">
                        <img
                          src={`${NEXT_PUBLIC_IMAGE_PREVIEW}/${formData.customer_aadhar}`}
                          alt="Aadhar preview"
                  ...
                  <div>
                    ...
                    {formData.customer_pancard && (
                      <div className="relative mt-4 w-full h-auto max-h-48 overflow-hidden rounded-md shadow-sm border border-gray-200">
                        <img
                          src={`${NEXT_PUBLIC_IMAGE_PREVIEW}/${formData.customer_pancard}`}
                          alt="PAN preview"
                          className="w-full h-full object-contain"
                        />
                      </div>
